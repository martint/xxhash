...
import static jdk.incubator.vector.LongVector.SPECIES_512;
...
class LongHashVector512
{
    private static final VectorShuffle<Byte> BYTE_SHUFFLE = VectorShuffle.fromValues(ByteVector.SPECIES_512,
    ...
    {
        for (int i = 0; i < SPECIES_512.loopBound(accumulators.length); i += SPECIES_512.length()) {
            LongVector accumulatorsVector = LongVector.fromArray(SPECIES_512, accumulators, i);
            ByteVector inputVector = ByteVector.fromArray(ByteVector.SPECIES_512, input, offset + i * 8);
            ByteVector secretVector = ByteVector.fromArray(ByteVector.SPECIES_512, SECRET, secretOffset + i * 8);
    ...
    {
        for (int i = 0; i < SPECIES_512.loopBound(accumulators.length); i += SPECIES_512.length()) {
            LongVector vector = LongVector.fromArray(SPECIES_512, accumulators, i);
            LongVector secret = ByteVector.fromArray(ByteVector.SPECIES_512, SECRET, (SECRET.length - STRIPE_LENGTH) + (i * 8)).reinterpretAsLongs();
