...
import static jdk.incubator.vector.LongVector.SPECIES_256;
...
class LongHashVector256
{
    private static final VectorShuffle<Byte> BYTE_SHUFFLE = VectorShuffle.fromValues(ByteVector.SPECIES_256,
    ...
    {
        for (int i = 0; i < SPECIES_256.loopBound(accumulators.length); i += SPECIES_256.length()) {
            LongVector accumulatorsVector = LongVector.fromArray(SPECIES_256, accumulators, i);
            ByteVector inputVector = ByteVector.fromArray(ByteVector.SPECIES_256, input, offset + i * 8);
            ByteVector secretVector = ByteVector.fromArray(ByteVector.SPECIES_256, SECRET, secretOffset + i * 8);
    ...
    {
        for (int i = 0; i < SPECIES_256.loopBound(accumulators.length); i += SPECIES_256.length()) {
            LongVector vector = LongVector.fromArray(SPECIES_256, accumulators, i);
            LongVector secret = ByteVector.fromArray(ByteVector.SPECIES_256, SECRET, (SECRET.length - STRIPE_LENGTH) + (i * 8)).reinterpretAsLongs();
